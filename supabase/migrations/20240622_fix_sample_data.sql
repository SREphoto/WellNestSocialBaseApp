...
-- Add sample moods with valid mood_type values
INSERT INTO public.moods (id, user_id, mood_type, message, created_at)
VALUES
  (gen_random_uuid(), '00000000-0000-0000-0000-000000000001', 'happy', 'Feeling at peace after my morning meditation session.', NOW() - INTERVAL '2 hours'),
  (gen_random_uuid(), '00000000-0000-0000-0000-000000000002', 'content', 'Just finished a good book. Feeling relaxed.', NOW() - INTERVAL '3 hours'),
  (gen_random_uuid(), '00000000-0000-0000-0000-000000000003', 'neutral', 'Regular day at work. Nothing special.', NOW() - INTERVAL '5 hours'),
  (gen_random_uuid(), '00000000-0000-0000-0000-000000000004', 'sad', 'Bit stressed about the upcoming deadline.', NOW() - INTERVAL '1 hour'),
  (gen_random_uuid(), '00000000-0000-0000-0000-000000000005', 'stressed', 'Just got some unexpected news. Processing it.', NOW() - INTERVAL '30 minutes'),
  (gen_random_uuid(), '00000000-0000-0000-0000-000000000001', 'content', 'Had a nice walk in the park. Fresh air does wonders.', NOW() - INTERVAL '1 day'),
  (gen_random_uuid(), '00000000-0000-0000-0000-000000000002', 'stressed', 'Need to talk to someone. Having a difficult time.', NOW() - INTERVAL '2 days'),
  (gen_random_uuid(), '00000000-0000-0000-0000-000000000003', 'happy', 'Weekend getaway was exactly what I needed.', NOW() - INTERVAL '3 days');
...
SELECT 
  ...
  m.id,
  CASE 
    WHEN m.mood_type = 'happy' OR m.mood_type = 'content' THEN 'So happy to see you doing well! 💚'
    WHEN m.mood_type = 'neutral' THEN 'Hope your day gets better! 🌻'
    WHEN m.mood_type = 'sad' OR m.mood_type = 'stressed' THEN 'Here if you need to talk. 💙'
  END,
